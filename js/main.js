...
(() => {
  ...
  console.log('DOM refs check:', {
    tracklistEl: !!tracklistEl,
    tracklistEmptyEl: !!tracklistEmptyEl,
    trackCountEl: !!trackCountEl,
    totalDurationEl: !!totalDurationEl
  });
  ...
  const minimizePlayerBtn = $('#minimizePlayerBtn');
  ...
  async function loadTracks(){
    ...
    if (!tracklistEl) {
      console.error('tracklistEl not found! DOM not ready?');
      return;
    }
    try {
  ...
  function playAudio(id){
    ...
    playerCol.classList.add('is-active');

    npTitle.textContent = displayTitle(track);
    npArtist.textContent = track.artist;
    console.log('Loading cover art:', track.cover_art);
    discArt.innerHTML = `<img src="${track.cover_art}" alt="Ảnh bìa ${escapeHtml(track.title)}" onerror="console.error('Failed to load cover:', this.src); this.remove();">
  ...
  // Close player button - stop and hide player
  closePlayerBtn.addEventListener('click', (e) => {
    ...
    discEl.classList.remove('is-spinning');
    mobileBar.hidden = true;
    playerCol.classList.remove('is-minimized');
    playerCol.classList.remove('is-active');
    updateMiniStatus();
  });

  // Minimize player button - collapse player panel (desktop only)
  minimizePlayerBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    if (window.matchMedia('(max-width:1080px)').matches) {
      playerCol.classList.remove('is-expanded');
    } else {
      // On desktop, hide the player panel content
      playerCol.classList.add('is-minimized');
    }
  });

  // Click on minimized player to expand (desktop only)
  playerCol.addEventListener('click', (e) => {
    if (playerCol.classList.contains('is-minimized') && 
        !window.matchMedia('(max-width:1080px)').matches &&
        !e.target.closest('.player-panel__header')) {
      playerCol.classList.remove('is-minimized');
    }
  });
